...
<div class="flex justify-center">
    ...
    <div id="card-overflow" class="w-[460px] bg-[#f5fbf7] h-screen px-4 pt-4 pb-[20px] relative overflow-y-auto">
        ...
        <div class="grid mt-3 gap-2">
            <iconify-icon onclick="window.location.href='{{ route('kolektor.dashboard-kolektor') }}'"
                class="cursor-pointer" icon="icon-park-outline:back" width="24" height="24">
            </iconify-icon>
...
<script>
    $(document).ready(function() {
        ...
        $("#submit-form").click(function(e) {
            ...
            const route = "{{ route('kolektor.upload-bukti', ['uuid' => ':uuid']) }}".replace(
                ":uuid",
                "{{ $transaksi->uuid }}"
            );
                ...
                success: function(response) {
                    if (response.success) {
                        Swal.fire({
                            ...
                            const route =
                                "{{ route('kolektor.proses-transaksi', ['uuid' => ':uuid']) }}"
